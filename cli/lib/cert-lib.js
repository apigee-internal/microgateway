...
CertLogic.prototype.checkCertWithPassword = function(options, callback) {
    let uri = util.format('%s/v1/organizations/%s/environments/%s/keyvaluemaps/%s',
...
CertLogic.prototype.checkPrivateCert = function(options, callback) {
    let uri = util.format('%s/v1/organizations/%s/environments/%s/keyvaluemaps/%s/entries/private_key',
        this.managementUri, options.org, options.env, this.vaultName);
...
CertLogic.prototype.generateKeysWithPassword = function generateKeysWithPassword(options, cb) {
    ...
    async.series([
        ...
        let key = results[0];
        let secret = results[1];
        let keys = {
            key: key,
            secret: secret,
        };

        let credentialUrl = util.format(managementUri, 'credential', options.org, options.env);
        ...
        request({
            ...
            if (res.statusCode >= 200 && res.statusCode <= 202) {
                let regionUrl = util.format(managementUri, 'region', options.org, options.env);
                ...
                request({
                    ...
                    if (res.statusCode >= 200 && res.statusCode <= 202) {
                        ...
                        let bootstrapUrl = util.format(managementUri, 'bootstrap', options.org, options.env);
                        let parsedUrl = url.parse(bootstrapUrl);
                        parsedUrl.host = res.body.host; // update to regional host
                        let updatedUrl = url.format(parsedUrl); // reconstruct url with updated host
...
function deleteVault(credentials, managementUri, organization, environment, vaultName, cb) {
    ...
    let uri = util.format('%s/v1/organizations/%s/environments/%s/keyvaluemaps/%s', managementUri, organization, environment, vaultName);
...
function createVault(credentials, managementUri, organization, environment, vaultName, entries, cb) {
    ...
    let uri = util.format('%s/v1/organizations/%s/environments/%s/keyvaluemaps', managementUri, organization, environment);
